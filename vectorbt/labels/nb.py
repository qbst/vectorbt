...
文件设计逻辑和作用概述：
本文件是vectorbt量化交易框架中专门用于生成机器学习训练标签的高性能计算模块。
...
设计警告：
⚠️ 重要提醒：本模块中的所有函数都包含前瞻性偏差（Look-ahead Bias），
这些函数使用了未来的数据信息，因此：
- 仅适用于离线训练机器学习模型
- 绝对不能用于实时交易系统
- 用于回测时需要确保没有引入未来信息泄露
...
应用场景：
- **股价预测模型**：生成股价上涨/下跌的分类标签
- **趋势识别模型**：创建趋势方向和强度的回归标签
- **突破检测模型**：生成价格突破事件的二分类标签
- **风险预测模型**：基于未来波动率创建风险等级标签
- **量化选股模型**：生成股票未来表现的排序标签

技术特点：
- **Numba JIT编译**：所有核心算法都经过JIT编译优化，性能提升10-100倍
- **矢量化处理**：支持2维数组批量处理，可同时处理多个资产的时间序列
- **灵活阈值设置**：支持标量、数组等多种阈值参数格式
- **多种趋势模式**：提供二元、连续、百分比等多种标签生成模式
- **内存高效**：使用就地操作和缓存友好的数据访问模式

与vectorbt生态系统的关系：
- 为indicators模块提供前瞻性指标计算支持
- 与enums模块协作处理趋势模式枚举
- 使用base.reshape_fns模块的灵活选择功能
- 与generic.nb模块共享通用的数值计算函数

该模块是vectorbt框架中连接传统技术分析与现代机器学习的重要桥梁，
为量化交易中的预测模型训练提供了工业级的标签生成解决方案。
...
# 1. 生成未来5天平均价格标签
future_mean = vbt.labels.nb.future_mean_apply_nb(
...
# 2. 生成固定期间收益率标签
fixed_labels = vbt.labels.nb.fixed_labels_apply_nb(
    prices.values.reshape(-1, 1), 
    n=5
)

# 3. 生成趋势方向标签
trend_labels = vbt.labels.nb.trend_labels_apply_nb(
    prices.values.reshape(-1, 1),
    pos_th=0.02,  # 2%上涨阈值
    neg_th=0.02,  # 2%下跌阈值
...
# 4. 生成突破信号标签
breakout_labels = vbt.labels.nb.breakout_labels_nb(
    prices.values.reshape(-1, 1),
    window=10,
    pos_th=0.03,  # 3%上涨突破阈值
    neg_th=0.03,  # 3%下跌突破阈值
    wait=1
)
```

注意事项：
- 所有函数的close参数都应该是2维数组，形状为(时间, 资产)
- wait参数控制标签的时间偏移，wait=1表示排除当前值
- 阈值参数支持灵活的广播规则，可以是标量、1维或2维数组
- 函数返回的标签数组与输入数组形状相同
================================================================================
"""
...
def bn_cont_trend_labels_nb(close: tp.Array2d,  # 输入的价格数组
    ...
    返回值：
        ...
        - 0: 表示将会上涨（当前价格接近区间最低点）
        - 1: 表示将会下跌（当前价格接近区间最高点）
        - 中间值: 表示上涨或下跌的程度
        - NaN: 极值点之外的区域
