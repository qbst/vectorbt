...
文件设计逻辑和作用概述：
本文件是vectorbt量化交易框架中专门用于技术指标计算的高性能核心模块。该模块提供了一整套
经过Numba JIT编译优化的技术指标计算函数，这些函数是vectorbt技术指标系统的计算引擎，
...
应用场景：
- **量化策略开发**：为技术分析策略提供高性能指标计算
- **实时交易系统**：支持毫秒级的实时指标更新
- **回测系统**：大规模历史数据的批量指标计算
- **风险管理**：波动率、趋势强度等风险指标的实时监控
- **多资产分析**：同时计算数百个资产的技术指标

性能特点：
- **编译时优化**：Numba JIT编译器进行代码优化，首次调用后达到最优性能
- **缓存友好**：内存访问模式优化，提高CPU缓存命中率
- **向量化操作**：利用SIMD指令集实现数据并行处理
- **零拷贝操作**：尽可能避免数据复制，使用视图和引用

与vectorbt生态系统的关系：
- **指标工厂集成**：通过IndicatorFactory自动集成到指标类中
- **缓存系统协作**：与vectorbt的全局缓存机制协同工作
- **类型兼容**：与vectorbt的类型系统完全兼容
- **装饰器支持**：通过attach_nb_methods装饰器自动暴露给用户

使用约定：
- 所有函数的第一个参数必须是2维NumPy数组
- 传递给函数的参数必须是Numba兼容类型
- 缓存函数返回字典，应用函数使用缓存结果
- 函数名以_nb结尾表示这是Numba编译的函数

该模块是vectorbt技术指标系统的核心，为量化交易中的技术分析提供了工业级的
高性能计算支持，是实现"高性能量化分析"的关键技术组件。
