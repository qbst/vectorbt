...
文件设计逻辑：
本文件定义了vectorbt技术指标系统中的通用配置对象，用于标准化和统一化指标计算时的参数处理行为。
通过预定义的配置模板，确保所有技术指标在处理多维数据时具有一致的行为模式。
...
使用场景：
- 技术指标工厂（IndicatorFactory）中的参数配置
- 多维时间序列数据的参数广播
- 批量指标计算的参数管理
- 内存优化的大规模数据处理

架构位置：
作为indicators模块的基础配置层，为basic.py、factory.py等模块提供统一的配置标准。
================================================================================

技术指标的通用配置定义模块。

本模块提供了技术指标系统中参数处理的标准化配置，确保所有指标在处理
多维数据时具有一致和高效的行为模式。
"""
